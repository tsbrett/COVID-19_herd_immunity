...
def create_polymod_matrix():
    ...
    # Note the transpose w.r.t original Mossong matrix to match SEIR model
    # notation
    cm = cm.transpose()
    ...
    ages_df[["sex",
             "year",
             "age"]] = ages_df.loc[:, "index"].str.split(pat="_", n=-1,
                                                         expand=True)

    ages_df["age_bins"] = pd.cut(ages_df["age"].astype("int"),
                                 bins=np.concatenate([np.arange(0, 71, 5),
                                                      [100]]),
                                 right=False)
