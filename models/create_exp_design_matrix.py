...
def create_exp_design_matrix(par):

    def get_r0_wrapper(x):
        return h.get_intervention_R0(par=par, var_par=x, sd_on=True)

    # Dict of self-isolation probability ("prob_s") and engagement
    # ("comp_factor") levels
    ...
    exp_design["si_eff"] = 1-exp_design.apply(h.get_self_isolation_factor,
                                              axis=1)
